                           ...
                           dig = 4, ppt = 20)
{
  ...
  win.graph(7, 5)
  plot(xa, dchisq(xa, df), type = "n", xlab = "Chi-square Statistic",
  ...
  if (side == "up" | grepl(side, "greater")) {
    ...
    cord.x = c(stat, seq(stat, prng[2], length.out = ppt),
               prng[2])
    cord.y = c(0, dchisq(seq(stat, prng[2], length.out = ppt),
                         df), 0)
    ...
    text(stat, staty, labels = bquote(chi[0]^2 == .(round(stat,
                                                          dig))), pos = postat, col = 4, cex = 1)
    text(rej, rejy, labels = bquote(chi[.(paste(1 - alp,
                                                ";", df))]^2 == .(round(rej, dig))), pos = 3,
         col = 2, cex = 1)
  }
  else if (side == "low" | grepl(side, "less")) {
    pv = plow
    cat("P-v =", round(pv, dig), "\n")
    cord.x = c(prng[1], seq(prng[1], stat, length.out = ppt),
               chis)
    cord.y = c(0, dchisq(seq(prng[1], stat, length.out = ppt),
                         df), 0)
    polygon(cord.x, cord.y, col = "lightcyan")
    segments(stat, 0, stat, dchisq(stat, df), lwd = 2, col = 2)
    text(stat, dchisq(stat, df), round(pv, dig), pos = 2,
         col = 2)
    ...
    cord.x = c(mup, seq(mup, prng[2], length.out = ppt),
               prng[2])
    cord.y = c(0, dchisq(seq(mup, prng[2], length.out = ppt),
                         df), 0)
    polygon(cord.x, cord.y, col = "lightcyan")
    cord.x = c(prng[1], seq(prng[1], mlow, length.out = ppt),
               mlow)
    cord.y = c(0, dchisq(seq(prng[1], mlow, length.out = ppt),
                         df), 0)
    polygon(cord.x, cord.y, col = "lightcyan")
    segments(c(mlow, mup), 0, c(mlow, mup), dchisq(c(mlow,
                                                     mup), df), lwd = 2, col = 2)
    text(c(mlow, mup), dchisq(c(mlow, mup), df), round(pv/2,
                                                       dig), pos = c(2, 4), col = 2)
    text(c(mlow, mup), 0, round(c(mlow, mup), dig), pos = 1,
         col = 4)
  }
