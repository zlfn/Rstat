...
cont.trans <- function (fx, TF, FTF, a, b, lo = 0, up = 1, plot = FALSE, ...)
{
  ...
  if (plot == TRUE) {
    ...
    for (k in 1:N) {
      ...
      ym = FTF[[k]](xm) * 0.5
      xa = k1:k2/200
      plot(xa, FTF[[k]](xa), type = "n", las = 1,
           ylab = paste0("f(", vn[k], ")"),
           xlab = "", main = paste0("pdf of ",
                                    Vn[k]))
      cord.x = c(a2, seq(a2, b2, 0.01), b2)
      cord.y = c(0, FTF[[k]](seq(a2, b2, 0.01)), 0)
      polygon(cord.x, cord.y, col = "lightcyan")
      text(xm, ym, labels = paste0("P(", a2, "<",
                                   Vn[k], "<", b2, ")\n=", round(py[k],
                                                                 4)), ...)
