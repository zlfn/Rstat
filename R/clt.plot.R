...
#'
#' @export
clt.plot <- function (dist, para, para2, ns = c(10, 30, 50), d = rep(0.5,
                                                         3), N = 10000, seed = 9857, sigknow = TRUE)
{
  dlist = c("exp", "gamma", "weibull", "beta",
            "norm", "t", "chisq", "f", "pois",
            "binom")
  dname = c("Exp", "Gam", "Wei", "Beta",
            "Norm", "T", "Chisq", "F", "Poi",
            "Binom")
  if (missing(dist)) {
  ...
  mt = ifelse(dnum %in% c(1, 6, 7, 9, 10), paste0(dname[dnum],
                                                  "(", para, ")"), paste0(dname[dnum], "(",
                                                                          para, ",", para2, ")"))
  if (dnum == 10)
    mt = paste0(dname[dnum], "(n,", para, ")")
  m = length(ns)
  zs = list()
  for (k in 1:m) zs[[k]] = genstat(dist, para, para2, ns[k],
                                   N, seed, sigknow)
