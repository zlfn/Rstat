                      ...
                      xp2)
{
  ...
  parnum = grep(pn, c("lambda", "alpha", "beta",
                      "theta", "mu", "sigma", "nu"))
  if (length(parnum) == 0)
    stop("Error in Parameter Names ...")
  lab = list()
  for (k in 1:N) lab[[k]] = switch(parnum, bquote(lambda ==
  ...
  dlist = c("exp", "gamma", "weibull", "beta",
            "norm", "t", "chisq", "f")
  dname = paste0(c("Exponential", "Gamma", "Weibull",
  ...
  cdf = dum$cdf
  if (missing(dcol))
    dcol = c("red", "blue", "orange2",
             "green4", "purple", "cyan2")
  win.graph(9, 5)
  par(mfrow = c(1, 2))
  if (missing(ymax))
    ymax = max(pdf)
  plot(xa, pdf[, 1], type = "l", main = mt1, lwd = 2,
  ...
  plot(xa, cdf[, 1], type = "l", main = mt2, lwd = 2,
