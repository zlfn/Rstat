...
chitest.plot <- function (stat, df, prng, side = "two", mt, dig = 4, ppt = 20)
{
  ...
  win.graph(7, 5)
  plot(xa, dchisq(xa, df), type = "n", xlab = "Test Statistic(x)",
       ylab = "f(x)", ylim = c(-0.1, 1) * max(dchisq(xa,
                                                     df)), main = mt)
  plow = pchisq(stat, df)
  if (any(grepl(side, c("greater", "up")))) {
    pv = 1 - plow
    cat(paste0("Chi0 = ", round(stat, dig), "\t P-v = ",
               round(pv, dig)), "\n")
    ...
    text(stat, dchisq(stat, df) * 0.9, round(pv, 4), pos = 4,
         col = 2)
    ...
    polygon(cord.x, cord.y, col = "lightcyan")
    segments(stat, 0, stat, dchisq(stat, df), lwd = 2, col = 2)
    text(stat, dchisq(stat, df) * 0.9, round(pv, 4), pos = 2,
         col = 2)
    text(stat, 0, round(stat, 4), pos = 1, col = 4)
  }
    ...
    segments(c(mlow, mup), 0, c(mlow, mup), dchisq(c(mlow,
                                                     mup), df), lwd = 2, col = 2)
    text(c(mlow, mup), dchisq(c(mlow, mup), df) * 0.9, round(pv/2,
                                                             4), pos = c(2, 4), col = 2)
    text(c(mlow, mup), 0, round(c(mlow, mup), 4), pos = 1,
         col = 4)
  }
  abline(h = 0)
