          ...
          dig = 4, prt = "exp")
{
  ...
  if (prt %in% c("cov", "cor")) {
    cat(paste0("Var(X) = ", round(Ex2, dig), " - ",
               round(abs(Ex), dig), "² = ", round(Vx, dig)),
        "\n")
    cat(paste0("Var(Y) = ", round(Ey2, dig), " - ",
               round(abs(Ey), dig), "² = ", round(Vy, dig)),
        "\n")
    cat(paste0("Cov(X,Y) = ", round(Exy, dig), " - ",
               round(Ex, dig), " × ", round(Ey, dig), " = ",
               round(Vxy, dig)), "\n")
  }
  if (prt == "cor") {
    cat(paste0("Corr(X,Y) = ", round(Vxy, dig), " / √(",
               round(Vx, dig), "×", round(Vy, dig), ") = ",
               round(Cxy, dig)), "\n")
  }
  out = list(Ex = Ex, Dx = Dx, Ey = Ey, Dy = Dy, Exy = Exy,
             Vxy = Vxy, Cxy = Cxy)
  invisible(out)
}
