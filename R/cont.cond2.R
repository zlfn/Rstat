...
cont.cond2 <- function (FUN, xc, yc, xs, ys, lo, up)
{
  ...
  win.graph(7, 4)
  par(mar = c(3, 4, 4, 2))
  plot(da, cpdf(da), type = "l", main = paste0("Conditional PDF of ",
                                               Dn, " | ", Cn, "=", Cs), ylim = c(0, max(cpdf(da)) *
                                                                                   1.1), xlab = "", ylab = yla, lwd = 3, col = 2)
  if (!missing(xs)) {
    fxv = cpdf(xs)
    segments(lo + 0.07 * (up - lo), fxv, xs, fxv, lty = 2,
             col = 4)
    text(rep(lo, length(xs)), fxv, round(fxv, 3), col = 4)
  }
  if (!missing(ys)) {
    fyv = cpdf(ys)
    segments(lo + 0.07 * (up - lo), fyv, ys, fyv, lty = 2,
             col = 4)
