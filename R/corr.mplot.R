...
corr.mplot <- function (X, item, xl, yl, mt, step = 1:4, alp = 0.05, dig = 4)
{
  ...
  if (1 %in% step) {
    ...
    if (missing(mt))
      mt = paste0("Scatter Plot (", letters[1:nv],
                  ") ", item)
    if (missing(xl))
      xl = "group1"
    if (missing(yl))
    ...
    par(mfrow = c(nr, nc))
    for (k in 1:nv) {
      plot(X[[k]][, 1], X[[k]][, 2], pch = 19, main = mt[k],
           xlab = xl, ylab = yl)
  ...
  if (2 %in% step) {
    ...
    for (k in 1:nv) {
      ...
      cat(paste0(item[k], "\t Corr(X1,X2) = ", round(cov12[k],
                                                     dig), " / √(", round(var1[k], dig), " × ",
                 round(var2[k], dig), ") = ", round(cor12[k],
                                                    dig)), "\n")
    }
  }
  ct = list()
  for (k in 1:nv) ct[[k]] = cor.test(X[[k]][, 1], X[[k]][,
                                                         2], conf.level = 1 - alp)
  if (3 %in% step) {
    cat("[Step 3] Correlation tests ------------------------\n")
    for (k in 1:nv) {
      cat(item[k], "\t Corr =", round(ct[[k]]$est,
                                      dig), " \t t-Stat =", round(ct[[k]]$stat,
                                                                  dig), "\t P-v =", round(ct[[k]]$p.val,
                                                                                          dig), "\n")
    }
  }
  if (4 %in% step) {
  ...
  if (5 %in% step) {
    ...
    for (k in 1:nv) {
      ...
      ttest.plot(ct[[k]]$stat, ct[[k]]$para, prng = c(-mr,
                                                      mr), dig = 2, mt = item[k], pvout = F)
    }
  }
}
